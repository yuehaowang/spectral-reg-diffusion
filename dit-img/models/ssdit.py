...
class NeuralEFProjector(nn.Module):
    def __init__(self, hidden_size, dims, bn=True, adaln=False):
        ...
        if dims[0] == 0:
            ...
            if len(dims) > 1:
                sizes = [hidden_size] + dims
                layers = []
                for i in range(len(sizes) - 2):
                    layers.append(nn.Linear(sizes[i], sizes[i + 1], bias=False))
                    if bn:
                        layers.append(_BatchNorm1d(sizes[i + 1]))
                    layers.append(nn.ReLU(inplace=True))
                layers.append(nn.Linear(sizes[-2], sizes[-1], bias=True))
                if bn:
                    layers.append(_BatchNorm1d(sizes[-1]))
        ...
        if adaln:
            ...
            self.norm_layer = nn.LayerNorm(hidden_size, elementwise_affine=False, eps=1e-6)

    def forward(self, z, c=None):
        if self.use_adaln:
            shift, scale = self.adaLN_modulation(c).chunk(2, dim=1)
            z = modulate(self.norm_layer(z), shift, scale)
...
class LabelEmbedder(nn.Module):
    def __init__(self, num_classes, hidden_size, dropout_prob):
        ...
        self.embedding_table = nn.Embedding(
            max(num_classes + use_cfg_embedding, 1), hidden_size
        )
...
class DiT_Llama(nn.Module):
    def __init__(
        ...
        self.projector = NeuralEFProjector(hidden_size=dim, dims=projector_dims, bn=projector_bn, adaln=projector_adaln)
    ...
    def forward(self, x, t, y=None, return_projector_output=False):
        self.freqs_cis = self.freqs_cis.to(x.device)

        if self.num_classes == 0 and y is None:
            y = torch.ones(x.shape[0]) * (self.y_embedder.embedding_table.num_embeddings - 1)
        ...
        for i, layer in enumerate(self.layers):
            ...
            if return_projector_output:
                if (i + 1) == self.encoder_depth:
                    zs = self.projector(x, adaln_input)

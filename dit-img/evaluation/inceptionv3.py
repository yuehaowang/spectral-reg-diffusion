...
# InceptionV3 weights converted from the official TensorFlow weights (torch-fidelity).
#   Original weights distributed under Apache License 2.0: https://github.com/tensorflow/models/blob/master/LICENSE
# Downloaded + cached by torch.hub; override with a local file via the
# INCEPTION_WEIGHTS environment variable for offline use.
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if feature_extractor_weights_path is None:
            feature_extractor_weights_path = os.environ.get("INCEPTION_WEIGHTS")
        with redirect_stdout(sys.stderr):
            if feature_extractor_weights_path is None:
                state_dict = load_state_dict_from_url(URL_INCEPTION_V3, progress=True)
            else:
                state_dict = torch.load(
                    feature_extractor_weights_path, weights_only=False
                )

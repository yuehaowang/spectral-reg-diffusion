...
from inceptionv3 import FeatureExtractorInceptionV3

from scipy import linalg
from tqdm.auto import tqdm


def compute_metrics(ref_batch, sample_batch):
    ref_batch_local = ref_batch
    sample_batch_local = sample_batch

    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    evaluator = Evaluator(device=device)

    ref_acts = evaluator.read_activations(ref_batch_local)
    ref_stats, ref_stats_spatial = evaluator.read_statistics(ref_batch_local, ref_acts)

    sample_acts = evaluator.read_activations(sample_batch_local)
    sample_stats, sample_stats_spatial = evaluator.read_statistics(
        sample_batch_local, sample_acts
    )
    ...
    sample_batch_dir = os.path.dirname(sample_batch)
    sample_batch_name = os.path.basename(sample_batch)
    metrics_filename = os.path.join(
        sample_batch_dir, f"metrics_{os.path.splitext(sample_batch_name)[0]}.json"
    )
...
class Evaluator:
    def __init__(
        ...
        self.inception = FeatureExtractorInceptionV3("inception_model", ["2048"]).to(
            device
        )
        self.inception.eval()
    ...
    def compute_activations(
        ...
        for batch in tqdm(batches):
            ...
            spatial_pred = (
                self.spatial_features.pop().movedim(1, -1)[..., :7].cpu().numpy()
            )
    ...
    def read_statistics(
        ...
        if "mu" in list(obj.keys()) and "mu_s" in list(obj.keys()):
    ...
    def compute_inception_score(
        ...
        for i in range(0, len(activations), self.softmax_batch_size):
            ...
            with torch.inference_mode():
                softmax_out.append(
                    acts.mm(self.inception.fc.weight.T).softmax(-1).cpu().numpy()
                )
...
class ManifoldEstimator:
    ...
    def __init__(
        ...
        """
        self.device = device
        self.distance_block = DistanceBlock(device)
    ...
    def evaluate_pr(
        ...
        features_1_status = np.zeros(
            [len(features_1), radii_2.shape[1]], dtype=np.bool_
        )
        features_2_status = np.zeros(
            [len(features_2), radii_1.shape[1]], dtype=np.bool_
        )
...
class DistanceBlock:
    ...
    def less_thans(self, batch_1, radii_1, batch_2, radii_2):
        dist32 = _batch_pairwise_distances(batch_1, batch_2)[..., None]
        return np.any(dist32 <= radii_2, axis=1), np.any(
            dist32 <= radii_1[:, None], axis=0
        )
